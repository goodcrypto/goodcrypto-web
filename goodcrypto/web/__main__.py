...
'''
    ...
    >>> import goodcrypto.utils
    
    >>> goodcrypto.utils.stop_host_system_services()

    >>> program = program_from_port(goodcrypto.constants.HTTP_PROXY_PORT)
    >>> assert program is None, ('{} already listening on http proxy port {}'.
    ...     format(program, goodcrypto.constants.HTTP_PROXY_PORT)))
    
    >>> goodcrypto.utils.start_host_system_services()
...
import httplib, os, sh, signal, socket, stat, threading
...
from goodcrypto.web.constants import USER, USER_GROUP, HTTP_PROXY_PORT, TOR_PORT
...
log = get_log('web.main.log', recreate=True)
...
class HttpProxy(Proxy):
    ...
    def is_up(self):
        ...
        ''' is there a good web test domain?
...
def profile():
    ''' Profile goodcrypto web. '''
    
    EXEC = 'goodcrypto.web.__main__.main()'
    PROFILE = '/tmp/goodcrypto.web.prof'
